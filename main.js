$(document).ready(function(){
  let number = $(".number");
  let display = $("#display .display-text");
  let decimalPoint = $("#decimal-point");
  let equal = $("#equal");
  let ac = $("#ac");
  let operator = $(".operator");
  let availableDecimalPoint = true; //小数点が使える状況かどうか 
  let secondNumber = false; //2個目の数字を入力待ちかどうか
  let afterEqual = false; //＝を押した直後か
  let availableOperator = false;//演算子を使える状態かどうか
  let operatorNumber = ""; //計算式のタイプを格納する変数（1=足し算、2=引き算、3=掛け算、4=割り算）
  let number1 = "";  //計算に使う数字を格納する変数
  let number2 = "";  //計算に使う数字を格納する変数
